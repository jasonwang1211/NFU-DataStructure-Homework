...
1. 遞迴版:直接照數學定義撰寫三條 if 判斷，交由系統自動建立呼叫堆疊。
- 特色:程式最簡潔，最能表現出 Ackermann 的遞迴結構。 
   當 $n$ 等於 1 或 0 時，直接返回結果，結束遞迴。

2. 非遞迴版（手寫堆疊）:使用一個固定長度的 int 陣列模擬 堆疊，將遞迴的「壓入」與「彈出」行為以 while 迴圈手動完成。 
- 特色:完全掌控堆疊深度，避免系統呼叫堆疊的大小限制（Stack overflow）。
...
1. 遞迴 vs. 手寫堆疊
- 遞迴版 為最直觀的實作，適合作為概念教學教材。
- 手寫堆疊版 則讓我實際體驗「把遞迴呼叫抽象成資料結構」的過程，學會了如何在程式中自行管理 堆疊深度，從而避免系統堆疊限制。
2. 未來可改進
